...
class ShiyanlouCoursesSpider(scrapy.Spider):
    ...
    def parse(self,response):
        ...
        print('##########################')
        for course in response.css('li.col-12.d-block.width-full.py-4.border-bottom.public.fork'):
            print('****************')
            
            print(course.css('div[class="d-inline-block mb-1"] h3 a::text').re_first('\s*(\w+)'))
            print('****************')
            yield {
